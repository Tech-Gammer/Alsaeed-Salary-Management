...
import 'LoginPage.dart'; // Make sure this import is correct
import 'auth_service.dart'; // Make sure this import is correct

class RegisterPage extends StatefulWidget {
  final VoidCallback? onToggleLogin; // Optional: To switch to login page

  const RegisterPage({super.key, this.onToggleLogin});
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      if (success) {
        ...
        // Navigate to LoginPage or use onToggleLogin if provided
        if (widget.onToggleLogin != null) {
        ...
        setState(() {
          _inlineErrorMessage = "Registration failed. Please try again or check your details.";
        });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Registration failed. The username or email might already be taken."),
      ...
      setState(() {
        _isLoading = false;
        _inlineErrorMessage = "An unexpected error occurred: ${e.toString()}";
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Widget _buildUsernameField() {
    return TextFormField(
      ...
      validator: (value) {
        ...
        // You might add more complex username validation (e.g., no spaces, regex)
        return null;
      },
  ...
  Widget _buildPasswordField() {
    return TextFormField(
      ...
      validator: (value) {
        ...
        // You could add more password strength rules here
        return null;
      },
      style: const TextStyle(color: textColor),
    );
  ...
  Widget _buildLoginToggle() {
    return Row(
      ...
      children: [
        ...
        TextButton(
          onPressed: widget.onToggleLogin ?? // Use the callback if provided
                  () {
                Navigator.pushReplacement( // Fallback to direct navigation
                  context,
                  MaterialPageRoute(builder: (_) => const LoginPage()),
                );
              },
